const RESETS = {
    prestige: {
        get require() { return player.fish.gte(1e5) }, 
        reset(force) {
            if (!force) {
    ...
    hadron: {
		...
		minimum_threshold: 21,
    },
}

function doReset(id, force, ...arg) {
    var r = RESETS[id]
    if (force || player.feature >= (r.minimum_threshold ?? 0) && r.require) r.reset(force, ...arg)
