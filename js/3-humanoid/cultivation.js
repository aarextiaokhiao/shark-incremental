const ORES = {
    'stone': {
        ...
        dense: 1,
        get mult() {
			let r = tmp.mining_tier_bonus[1] ?? E(1)
			...
			r = r.pow(tmp.mining_tier_bonus[8]??1)
			return r
		},
    },
    'coal': {
    ...
    'vibranium': {
        ...
        dense: 1e14,
        luck_penalty: 0.1,
        get mult() { return Decimal.pow(simpleResearchEffect('f6')).pow(tmp.mining_tier_bonus[8]??1) },
    },
...
const MINING_TIER = {
    ...
    get ascend_base() { return this.ascend_base_milestone.filter(x => player.humanoid.mining_ascend.gte(x)).length },
    get ascend_ore_generator() { return this.ascend_gen_milestone.filter(x => player.humanoid.mining_ascend.gte(x)).length},
}
...
function reloadOres() {
    ...
    var h_inc = tmp.mining_tier_bonus[0]??1
